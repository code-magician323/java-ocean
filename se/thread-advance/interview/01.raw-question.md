...
2. CAS: `自旋锁` + `UnSafe`
   ...
   public final class Unsafe {
    ...
    // 3. 线程 B 通过 getAndAddInt(var1, var2) 拿到 value 值为 3, 线程 B 没有挂起, 执行了 compareAndSwapInt 方法, 比较主内存中的值也是 3, 则// 成功修改主内存中的值为 4, 线程 B 结束
    // 4. 此时线程 A 恢复, 接着执行 compareAndSwapInt 方法进行比较发现获取的主内存的值为 4 不同于 var5[3], 说明该值被其他线程抢先异步修改了, 那么线// 程 A 本次修改失败, 只能重新读取重新来一遍*5*
...
1. 简介
   ...
   - 用来构建锁[ReentrantLock]和其他同步器组件[CountDownLatch/CyclicBarrier/Semapthore]的基石
